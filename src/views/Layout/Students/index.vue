...
const addStudent = async () => {
  ...
  // 切换分页到最后一页
  page.value.pageNow = Math.ceil(total.value / page.value.pageSize)
}
// 编辑
  ...
  timer.value = setTimeout(async () => {
    ...
    switch (selectV.value) {
      ...
      case 'sclass':
        ...
        break
      case 'ssex':
        page.value.ssex = selTemp.value === '男' ? '0' : '1'
        break
      default:
        break
    }
...
<template>
  <div class="w-full flex items-center bg-white rounded-t-lg mb-5">
    ...
    />
    <!-- 新增按钮 -->
    <a-button shape="circle" class="m-2" @click="showAddModal"
      ><PlusOutlined
    /></a-button>
  </div>
  <!-- 表格 -->
